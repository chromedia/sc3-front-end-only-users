...
function Insert_User_Profile($atts) {
	...
	foreach ($Fields as $Field) {
		if (!in_array($Field->Field_Name, $Omitted_Fields)) {
			...
			if ($Field->Field_Type != "picture") {$ReturnString .= "<div class='ewd-feup-text-input ewd-feup-user-profile-input'>" . $Value . "</div>";}
			else {$ReturnString .= "<img class='ewd-feup-text-input' src='" . $Value . "' alt='" . $Field->Field_Name . "'/>";}
			$ReturnString .= "</div>";
		}
	}

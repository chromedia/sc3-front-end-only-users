...
function Add_Edit_User() {
	...
	if ($UserCookie['Username'] != "" and ($_POST['action'] == "Add_User" or $_POST['ewd-feup-action'] == "register")) {
		$user_update = array("Message_Type" => "Error", "Message" => __("You are currently logged in. Please log out to create a new account.", "EWD_FEUP")); return $user_update;
	}
...
function Mass_Delete_EWD_FEUP_Payments() {
	...
	$update = __("Payments have been successfully deleted.", 'EWD_FEUP');
	$user_update = array("Message_Type" => "Update", "Message" => $update);
	return $user_update;
}

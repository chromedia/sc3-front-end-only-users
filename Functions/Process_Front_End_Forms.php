...
function Confirm_Login($Username = "", $WP_Login = "No") {
	...
	$Account_Expiry = strtotime($User->User_Account_Expiry);
	if ($Account_Expiry == "") {$Account_Expiry = 9446593304;}

	if($Passwords_Match) {
		if ($Payment_Frequency == "None" or ($User->User_Membership_Fees_Paid == "Yes" or $Account_Expiry < time())) {
...
function Forgot_Password() {
	...
	if( !empty( $User ) and $Validate_Captcha == "Yes")
	{
		...
		$mail_success = wp_mail( $User_Email, $subject, $message, $headers);
